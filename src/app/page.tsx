  ...
  return (
    <article ref={cardRef} data-product-id={product.id} className={`relative flex flex-col overflow-hidden rounded-xl border bg-white shadow-sm transition-all ${isOutOfStock ? 'border-gray-300 opacity-80' : 'border-gray-200 hover:shadow-md'}`}>
      <div className="absolute left-3 top-3 z-10 flex flex-col gap-1.5">
        {discount > 0 && (
          <span className="-rotate-2 rounded-lg border border-white/30 bg-gradient-to-r from-red-600 to-orange-500 px-3 py-1.5 text-xs font-extrabold text-white shadow-[0_4px_12px_rgba(220,38,38,0.45)] ring-1 ring-red-700/20">
      ...
      <div className="flex flex-grow flex-col justify-between p-5">
        <div>
          ...
          <h4 onClick={() => onOpenProduct(product.id)} className="mb-2 line-clamp-1 cursor-pointer text-lg font-bold text-gray-900 transition-colors hover:text-red-600">{product.name}</h4>
        ...
        <div>
          ...
          <div className="mb-2.5 flex items-baseline gap-2">
            <span className="text-3xl font-black tracking-tight text-red-600">{product.salePrice.toLocaleString('no-NO')} kr</span>
            {product.listPrice > product.salePrice && <span className="text-sm text-gray-400 line-through">{product.listPrice.toLocaleString('no-NO')} kr</span>}
          </div>
...
export default function HomePage() {
  ...
  const getStockInfo = (stock: number): StockInfo => {
    ...
    if (stock <= 5) return { text: `${stock} stk igjen`, wrapperClass: 'bg-orange-50 text-orange-700 border-orange-200', icon: <AlertTriangle className="h-4 w-4" aria-hidden="true" /> };
