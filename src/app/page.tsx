...
function ProductCard({
    ...
    const observer = new IntersectionObserver(
      ([entry]) => {
        ...
        void fetch('/api/products/view', {
          ...
          body: JSON.stringify({ productId: product.id, type: 'home' }),
          keepalive: true,
        }).catch((error) => {
          console.error(
  ...
  return (
    <article
      ...
      className={`relative flex flex-col overflow-hidden rounded-xl border bg-white shadow-sm transition-all ${
        isOutOfStock
    ...
    >
      <div className="absolute left-3 top-3 z-10 flex flex-col gap-1.5">
        ...
        {isNew && (
          <span className="flex items-center gap-1 rounded-md bg-emerald-600 px-2.5 py-1 text-xs font-bold text-white shadow-sm">
      ...
      <div
        ...
        className="group relative flex h-64 cursor-pointer items-center justify-center overflow-hidden border-b border-gray-100 bg-gray-50 p-6"
      >
        <img
          src={
      ...
      <div className="flex flex-grow flex-col justify-between p-5">
        <div>
          ...
          <h4
            onClick={() => onOpenProduct(product.id)}
            className="mb-2 line-clamp-1 cursor-pointer text-lg font-bold text-gray-900 transition-colors hover:text-red-600"
          >
            {product.name}
          </h4>
        ...
        <div>
          {product.pickupOnly ? (
            <div className="mb-3 flex items-center gap-1 rounded border border-amber-200 bg-amber-50 p-2 text-xs font-medium text-amber-800">
              <MapPin className="h-3.5 w-3.5" />
              Må hentes i butikk på Sortland
            </div>
          ) : (
            <div className="mb-3 flex items-center gap-1 rounded border border-blue-200 bg-blue-50 p-2 text-xs font-medium text-blue-800">
...
export default function HomePage() {
  ...
  useEffect(() => {
    if (!klarnaMessage) return;
    const timeout = window.setTimeout(() => setKlarnaMessage(''), 5000);
  ...
  return (
    <div className="flex min-h-screen flex-col bg-gray-50">
      <header className="z-50 bg-neutral-900 px-4 py-2 text-sm text-white">
        <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-2 md:flex-row">
          <div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-2">
            <span className="flex items-center gap-1"><MapPin className="h-4 w-4 text-red-600" />Verkstedveien 2, 8402 Sortland</span>
            <span className="flex items-center gap-1"><Phone className="h-4 w-4 text-red-600" />76 12 13 60</span>
            <span className="flex items-center gap-1"><Mail className="h-4 w-4 text-red-600" />sortland@eiksenteret.no</span>
          </div>
          <div className="text-xs text-gray-400">Org.nr: 936 858 031 | Eiksenteret Sortland</div>
        </div>
      </header>
      ...
      <nav className="sticky top-0 z-40 border-b border-gray-200 bg-white shadow-sm">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3">
          <div className="flex cursor-pointer items-center gap-4" onClick={() => router.push('/')}>
            <img src="/tilbudsbodenlogo.svg" alt="Tilbudsboden.no - fra Eiksenteret Sortland" className="h-10 object-contain md:h-12" />
            <div className="border-l border-gray-300 pl-4">
              <h1 className="text-lg font-bold leading-none tracking-tight text-gray-900 md:text-xl">Tilbudsboden.no</h1>
              <p className="mt-1 text-xs text-gray-500">Restpartier, kampanjevarer og gode kjøp fra Eiksenteret Sortland</p>
            </div>
          </div>
          <div className="hidden items-center gap-4 sm:flex">
            <span className="flex items-center gap-1 rounded-full border border-red-200 bg-red-50 px-3 py-1.5 text-xs font-semibold text-red-700">
              <Tag className="h-3.5 w-3.5" />Direkte fra lageret på Sortland
            </span>
          </div>
        </div>
      </nav>
      ...
      <section className="mx-auto w-full max-w-7xl px-4 pb-2 pt-4">
        <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-neutral-900 shadow-md">
          <img src="/EiksenteretSortland.png" alt="Eiksenteret Sortland Butikk" className="max-h-[280px] w-full bg-neutral-900 object-contain" />
          <div className="absolute inset-0 flex items-center bg-gradient-to-r from-neutral-950/90 via-neutral-950/60 to-transparent p-6 md:p-10">
            <div className="max-w-xl text-white">
              <span className="mb-2 inline-block rounded bg-red-600 px-3 py-1 text-xs font-bold uppercase tracking-widest text-white">Velkommen til Tilbudsboden.no</span>
              <h2 className="mb-2 text-xl font-extrabold tracking-tight text-white md:text-3xl">Restpartier, kampanjevarer og gode kjøp</h2>
              <p className="hidden text-xs leading-relaxed text-gray-200 sm:block md:text-sm">Her finner du restpartier, kampanjevarer, utstillingsmodeller og ekstra gode kjøp direkte fra lageret og butikken vår på Sortland.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <main className="mx-auto w-full max-w-7xl flex-grow px-4 py-10">
        {klarnaMessage && (
          <div role="status" className="mb-6 rounded-xl border border-[#ffb3c7] bg-[#fff0f5] p-4 text-center text-sm font-semibold text-gray-900">
            {klarnaMessage}
          </div>
        )}

        <div className="mb-8 flex items-center justify-between">
          <h3 className="flex items-center gap-2 text-2xl font-bold text-gray-900">Aktuelle tilbud</h3>
          <span className="text-sm text-gray-500">{products.length} varer tilgjengelig nå</span>
        </div>
          ...
          <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
            {products.map((product) => (
              <ProductCard
                ...
                onBuyWithKlarna={() =>
                  setKlarnaMessage(
                    'Klarna lanseres snart på Tilbudsboden.no. Betalingsvalget er foreløpig ikke aktivert.'
                  )
                }
      ...
      {selectedProduct && (
        <CheckoutModal product={selectedProduct} onClose={() => setSelectedProduct(null)} onSuccess={loadProducts} />
      )}
      ...
      <footer className="mt-12 border-t border-neutral-800 bg-neutral-900 py-12 text-sm text-gray-400">
        <div className="mx-auto grid max-w-7xl grid-cols-1 gap-8 px-4 md:grid-cols-3">
          <div>
            <div className="mb-4 inline-block rounded-lg bg-white p-2"><img src="/tilbudsbodenlogo.svg" alt="Tilbudsboden.no - fra Eiksenteret Sortland" className="h-8 object-contain" /></div>
          ...
          <div>
            ...
            <button onClick={() => router.push('/vilkar')} className="flex items-center gap-1 text-xs font-bold text-red-500 underline hover:text-red-400"><ShieldCheck className="h-4 w-4" />Les våre fullstendige Salgsvilkår</button>
          </div>
          <div>
